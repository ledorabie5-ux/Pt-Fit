...
export default function AddMemberTab({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      alert(isAr ? "فشل البحث. يرجى المحاولة مرة أخرى." : "Search failed. Please try again.");
    } finally {
      setSearching(false);
    }
  };
    ...
    <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 shadow-lg space-y-6 animate-in fade-in duration-200">
      <div>
        <h3 className="text-sm font-bold text-white flex items-center gap-2">
          ...
          {isAr ? "إضافة مشترك جديد" : "Add Member"}
        </h3>
        <p className="text-xs text-neutral-400 mt-1">
          {isAr
            ? "ابحث عن أي مستخدم مسجل عن طريق رقم هاتفه لربطه كعميل لديك وتفعيل اشتراكه."
            : "Search for any registered user by their phone number to link them as your client and activate their membership."}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 max-w-md">
        <input
          type="text"
          placeholder={isAr ? "مثال: 0501234567 أو رقم الهاتف" : "e.g. +12345678 or phone number"}
          value={phoneQuery}
      ...
      {searching && (
        <div className="flex items-center gap-2 py-4 text-xs text-neutral-400">
          ...
          {isAr ? "جاري البحث عن المستخدم..." : "Searching for user..."}
        </div>
      )}
      ...
      {!searching && hasSearched && results.length === 0 && (
        <div className="p-4 bg-neutral-950 rounded-lg border border-neutral-800 text-xs text-neutral-400 max-w-md">
          {isAr ? (
            <>لم يتم العثور على حساب برقم الهاتف "<span className="text-white font-semibold">{phoneQuery}</span>". يرجى التأكد من تسجيل حسابهم أولاً.</>
          ) : (
            ...
            <div key={trainee.uid} className="pt-4 first:pt-0 space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-xs text-neutral-400 mt-0.5">{trainee.email} • {trainee.phone || (isAr ? "لا يوجد رقم هاتف" : "No phone listed")}</p>
                  <p className="text-[9px] text-neutral-500 font-mono mt-1">
                    {isAr ? "معرّف المستخدم: " : "User ID: "}{trainee.uid}
                  </p>
                </div>
                <span className="text-xs bg-neutral-900 border border-neutral-800 text-neutral-400 px-2.5 py-1 rounded">
                  {isAr ? "الحالة: " : "Status: "}{trainee.status === "approved" ? (isAr ? "موافق عليه" : "APPROVED") : (isAr ? "قيد الانتظار" : "PENDING")}
                </span>
              </div>
