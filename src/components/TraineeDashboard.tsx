          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden shadow-xl min-h-[500px]">
            {loadingProgram ? (
              ...
              <div className="p-6">
                ...
                {activeTab === "diet" && (
                  <div className="space-y-6">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-neutral-950 p-4 rounded-xl border border-neutral-800">
                      <div>
                        ...
                        <h3 className="text-md font-bold text-white mt-0.5">{lang === "ar" ? "نظام التغذية والوجبات اليومية" : "Personalized Meal & Sports Nutrition"}</h3>
                      </div>

                      <div className="flex items-center gap-2 bg-neutral-900 p-1 rounded-xl border border-neutral-800">
                        <button
                          type="button"
                          onClick={() => setDietSubTab("assigned")}
                        ...
                        >
                          {lang === "ar" ? "الوجبات المعتمدة" : "Current Diet Plan"}
                        </button>
                        <button
                          type="button"
                          onClick={() => setDietSubTab("ai_generator")}
                          className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${
                            dietSubTab === "ai_generator"
                              ? "bg-emerald-600 text-white shadow"
                              : "text-neutral-400 hover:text-white"
                          }`}
                        >
                          <span>✨</span>
                          <span>{lang === "ar" ? "نظام التغذية الذكي AI" : "AI Sports Nutrition"}</span>
                        </button>
                      </div>
                    </div>
                    ...
                    {dietSubTab === "assigned" ? (
                      ...
                      <AINutritionGenerator
                        ...
                        initialHeight={currentUser.height || 175}
                        initialAge={currentUser.age || 25}
                        initialGender={(currentUser.gender === "Female" || currentUser.gender === "female") ? "female" : "male"}
                        initialGoal={currentUser.fitnessGoal || "Muscle Gain"}
                        lang={lang}
                        isSaving={isSavingAiPlan}
                        onSavePlanToProgram={async (newMeals) => {
                          setIsSavingAiPlan(true);
                          try {
                            const updatedWorkoutDays = program?.workoutDays || [];
                            ...
                            setDietSubTab("assigned");
                            alert(lang === "ar" ? "تم حفظ واعتماد نظام التغذية بالذكاء الاصطناعي بنجاح!" : "AI Nutrition plan successfully saved to your program!");
                          } catch (err) {
                            console.error("Failed to save AI diet plan:", err);
                          } finally {
